...
**The interval files' `within`/`structural_change` follow [`docs/decomposition.tex`](../docs/decomposition.tex) equation (3) — a nested, growth-rate-normalized version of the equation above, not the same level-form equation applied at coarser resolution.** Rather than treating the 3 broad sectors as the decomposition's own index (as the annual/full-period files still do), each broad sector's own within/structural-change split is built from GGDC's finer 9 sub-sectors, with sub-sector employment shares normalized by **country-total** employment (not each sub-sector's share of its own broad sector). The quantity actually decomposed for sector *b*, `LP_b = Σ_{i∈b} y_i·l_i`, works out to `Q_b/L_country` — sector *b*'s own contribution to country-wide productivity, a different quantity from its standalone productivity `Q_b/L_b` (still available separately, unchanged, and still what the `ln_productivity` level regression below uses). `within`/`structural_change` are the level-form terms divided by `LP_b` at the start of the interval — small, unitless growth-rate contributions, not raw productivity-level changes. Checked directly, not assumed: at 3-year resolution every one of the 24 countries has at least 5 usable bins; at 5-year resolution 2 of 24 (Angola, Sierra Leone) fall below 3 bins. Given how few are affected — and that both are resource-dependent economies directly relevant to the §6.4.D resource-dependence heterogeneity cut — the recommendation is to keep them in the baseline sample (a staggered panel estimator pools identifying variation across units via its fixed effects, so a thin country loses precision, not identification) and treat their exclusion as a robustness check rather than a default drop.
...
```
with country and year fixed effects, estimated first by TWFE as a baseline and then by the staggered-adoption-robust estimator (Callaway–Sant'Anna or the ETWFE approach of Sellner & Yotov) as the primary specification, given the staggered timing of African countries' entry into reciprocal agreements. `Within_c,t`/`Structural_c,t` are growth-rate contributions (eq. 3 above), so `β_W`/`β_S` are interpreted in percentage-point-of-growth terms, not raw productivity-level units. As a secondary heterogeneity cut (§6.4.D), re-run both equations comparing non-reciprocal-only spells against the reciprocal-treated group — if β_W is materially larger for the reciprocal case, that is direct evidence the input-access/competition channels (not just market access) are driving within-sector upgrading.

### 6.4 Empirical strategy for the underlying sector-level productivity regressions

**A. Descriptive event-study plots** of sectoral productivity around each country's first Northern-PTA entry, before any regression — the fastest check on pre-trends. **Done** — `notebooks/05_descriptive_event_study.ipynb`. Productivity *levels* show a pre-existing rising trend in all three sectors that accelerates post-treatment in Agriculture/Services but decelerates in Manufacturing (plausibly a short-run adjustment-cost pattern per §2.2–§2.3, not evidence against the design); the annual within-sector *growth rate* — the actual regression outcome — shifts from negative to positive after treatment in Manufacturing and Services specifically. Adoption clusters in two cohorts (1976–2000, 2007–2010), so any pooled event-time average over-represents the later cohort at long horizons — a reason to prefer a cohort-aware estimator (Callaway–Sant'Anna) over pooled TWFE, not a reason to distrust the descriptive check itself. Never-treated countries have similar median growth to treated countries (supporting their use as controls) but far more dispersion. Algeria has zero pre-treatment observations in this project's data and cannot be event-time-aligned. Full detail and all figures in the notebook; summary tables at `data/processed/descriptive_*.csv` (documented in `data/processed/README.md`).
...
**Done**, together with the §6.3 component regressions below — `notebooks/06_twfe_baseline.ipynb`. β = 0.149 (SE 0.121, p = 0.22, N = 3,288) — positive, directionally consistent with the descriptive event-study's finding that Agriculture/Services productivity accelerates post-treatment, but statistically insignificant. The 18 §6.3.A component-regression specifications (both interval widths, sector and country level, with switching-interval and short-series robustness variants) show completely stable signs — `within` positive, `structural_change` negative, in every specification — though none reach conventional significance (closest: p ≈ 0.063). Building this also surfaced and fixed a genuine numerical issue: `γ_cs` and `δ_st` dummy-encoded together produce an exactly rank-deficient design matrix (by `n_sectors - 1`, since both fixed-effect sets share the sector dimension), which leaves the point estimate correct but makes the standard error numerically unstable by orders of magnitude unless the exact redundant columns are identified (via a rank-revealing QR decomposition) and dropped before fitting. Full results at `data/processed/twfe_baseline_results.csv` (documented in `data/processed/README.md`). As expected per §2.8, this is a comparison point only — none of these numbers should be read causally.

**Sector-specific extension, also done**: all of the above pools `Reciprocal`'s coefficient across sectors (a single β, matching the literal equations). Interacting `Reciprocal` with sector dummies instead — same fixed-effect structure, three coefficients instead of one — shows **Manufacturing's `structural_change` effect negative and individually significant at 5-year resolution** (-0.221, p ≈ 0.036), the only individually significant coefficient in the sector-interacted results, with a significant joint test that the three sectors' `structural_change` effects differ at that resolution (F ≈ 3.66, p ≈ 0.042). Agriculture's `structural_change` effect runs the opposite sign (+0.064, p ≈ 0.083, not itself significant) — meaningful despite Agriculture mapping to a single GGDC sub-sector, since sub-sector employment shares are normalized by *country*-total employment (eq. 3 above), so a single sub-sector's country-relative share can still move. None of the sector-specific `within` coefficients reach individual significance at either interval width. Manufacturing's own annual `ln_productivity` effect stays marginal (p ≈ 0.057), unaffected by any of the interval-decomposition work since it comes from the annual panel directly. Full results at `data/processed/twfe_sector_interacted_results.csv`.

**Interval resolution for the `ln_productivity` baseline, also checked**: the §6.3.A component regressions use 3-year and 5-year intervals because differencing amplifies noise in flow variables, but the §6.4.B baseline above is a level, not a flow, so it was originally run only on the annual panel. Re-running it on each broad sector's own **standalone** productivity (`Q_b/L_b`, point-in-time at the calendar-grid edge year — *not* the `LP_b` "contribution" quantity §6.3's `within`/`structural_change` regressions use) at each 3-year/5-year interval shows interval smoothing does *not* clearly help here: the point estimate stays positive throughout (0.064 to 0.157) and never reaches significance, and precision is a wash rather than a clean improvement — 3-year sector-level is the one specification that's more precise than annual (SE 0.118 vs. 0.121), but 3-year country-level, 5-year sector-level, and 5-year country-level are all less precise, with 5-year country-level the worst of the five (SE 0.139). This matches the expectation that a level variable, unlike a difference, does not carry the noise that motivated interval-smoothing the flow components — confirmed directly rather than assumed.

**`LP_b` regressed directly, also done, on request**: `within`/`structural_change` use `LP_b` only as an internal normalizing denominator — `Reciprocal`'s effect on `LP_b` itself had never been estimated directly. Doing so shows a genuinely different result from the standalone-productivity regression just above, not a noisier version of it: both point estimates sit almost exactly at zero (+0.0004 at 3-year, p ≈ 0.998; -0.0148 at 5-year, p ≈ 0.920), against `productivity_b`'s own positive 0.157/0.129 at the same resolutions. Since `LP_b = productivity_b × employment_share_b` by construction, this is consistent with the employment-share side offsetting whatever positive productivity signal exists, though this specification alone can't say whether that reflects a genuine reallocation effect or noise in a two-part ratio — "does a sector produce more per worker" and "does that sector end up contributing more to the country's overall output per worker" get different answers here. Full results (both regressions) at `data/processed/twfe_interval_productivity_results.csv`.
...
7. Estimate the component regressions (§6.3.A) and the sector-level regressions (§6.4.B/C) with `Reciprocal_c,t` as the sole treatment; compare TWFE vs. staggered-DiD estimates directly, replicating the Sellner–Yotov bias finding in this project's own African sample. **TWFE half done** — `notebooks/06_twfe_baseline.ipynb`, see §6.4.B above. The staggered-DiD half — needed to actually run the comparison this step asks for — is not yet built; that's the next step.
