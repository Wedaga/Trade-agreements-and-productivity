...
3. Run the McMillan–Rodrik decomposition on the African panel *without* any trade variable first, to replicate de Vries–Timmer–de Vries / Diao–Harttgen–McMillan as a sanity check that the pipeline is correct. **Prerequisite done** — `notebooks/03_clean_ggdc_productivity.ipynb` aggregates the GGDC productivity panel's 9 sub-sectors down to the three broad sectors (Agriculture/Manufacturing/Services) the decomposition (§6.3) is defined over, following Herrendorf, Rogerson & Valentinyi (2014)'s own mapping, confirmed directly from their Data Appendix for this specific data source; output at `data/processed/ggdc_africa_broad_sectors.csv`. Not yet merged with the trade-agreement panel, and the decomposition itself has not yet been run.
