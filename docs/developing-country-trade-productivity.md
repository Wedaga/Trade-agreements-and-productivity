...
5. **Trade- or GDP-weighted exposure** as a further robustness variant of the same binary, weighting by each Northern partner's share of the African country's total trade or GDP where more than one Northern partner is involved, requiring bilateral trade data (BACI/Comtrade) as an additional input.
...
2. Build `Reciprocal_c,t` (§6.2) for these countries, and empirically check that it is (near-)absorbing — i.e., that it essentially never reverts from 1 back to 0 for any country in the sample — before committing to a single Callaway–Sant'Anna-style estimator as primary.
