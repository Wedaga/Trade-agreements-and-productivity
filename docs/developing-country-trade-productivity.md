...
**Callaway, B., Goodman-Bacon, A., & Sant'Anna, P. H. C. "Difference-in-Differences with a Continuous Treatment." NBER Working Paper 32117.** ★ *Method precedent*
Extends the staggered-adoption DiD framework above to a continuous or ordinal dose rather than a binary treatment, estimating an average causal response as a function of dose under a generalized parallel-trends assumption that must hold at every dose level. This, not standard Callaway–Sant'Anna, is the appropriate tool for this project's primary treatment variable — the full 0–6 `PTA_North_depth_c,t` score (§6.2) — since Baier–Bergstrand's agreement-depth measure is ordinal, not binary.

**de Chaisemartin, C., & D'Haultfœuille, X. (2020). "Two-Way Fixed Effects Estimators with Heterogeneous Treatment Effects." *American Economic Review*, 110(9), 2964–2996; extended in follow-up work to non-binary and non-staggered (switching) treatments.** ★ *Method precedent*
Shows TWFE regressions estimate a weighted sum of group-time treatment effects with potentially *negative* weights (so a regression coefficient can be negative even when every unit's true effect is positive), and proposes an alternative that generalizes the event-study approach by defining the event as the period a unit's treatment first *changes*, in either direction. Their extension to non-binary, non-staggered treatments is the appropriate estimator for the project's level-1 (non-reciprocal) treatment leg specifically, since preference-scheme eligibility (e.g., AGOA) is reviewed periodically and has genuinely been suspended and reinstated for real countries in the sample, breaking the absorbing-treatment assumption standard Callaway–Sant'Anna relies on.
...
**Does this design still support the staggered-DiD estimator proposed in §2.8? Yes, but it is now three related tools matched to three variables, not one tool applied uniformly — and the split surfaces a real complication a single "any agreement" binary had conveniently hidden.**

- **`Reciprocal_c,t` (binary)** is the cleanest fit for exactly the technique originally proposed (Callaway–Sant'Anna; Sellner & Yotov's ETWFE). It is staggered (African countries enter reciprocal agreements with Northern partners at different dates through the 2010s) and, for this sample, plausibly *absorbing* — once ratified, a reciprocal FTA/CU/CM/EUN essentially never reverts. If anything, this binary is a **better-suited application of the technique than the original "any agreement" design**: GSP eligibility was granted to most African countries in a short window around 1976, which bunches timing and weakens a staggered design's identifying variation, whereas reciprocal EU EPA entry is genuinely spread across years.
- **`PTA_North_depth_c,t` (the ordinal 0–6 score)** is *not* directly compatible with standard Callaway–Sant'Anna, which is built for a binary treatment. Two options: discretize into a stack of binary "at least level *k*" cutoffs and run Callaway–Sant'Anna separately at each threshold (a dose-response-by-cutoff approach), or use the purpose-built extension — **Callaway, Goodman-Bacon & Sant'Anna's "Difference-in-Differences with a Continuous Treatment"** (NBER Working Paper 32117), which extends the same staggered-adoption logic to continuous/ordinal doses, estimating an average causal response as a function of dose under a generalized parallel-trends assumption (one that must hold at *every* dose level, not just treated-vs-untreated). This is the right tool for the primary ordinal specification.
- **The level-1, non-reciprocal leg specifically is not always absorbing — this is the complication the redesign surfaces.** AGOA eligibility is reviewed annually and has genuinely been suspended and later reinstated for real countries in this sample: Madagascar's AGOA status was suspended in December 2009 after a coup and reinstated in 2014 — a documented drop from level 1 back to level 0 and back up again, not a one-way staggered adoption. Standard Callaway–Sant'Anna assumes a well-defined, absorbing "first treatment date" and is not built for units that switch in and out. For this leg, **de Chaisemartin & D'Haultfœuille's estimator family** (their 2020 *American Economic Review* paper, extended in follow-up work to non-binary *and* non-staggered/switching treatments) is the appropriate substitute — it is explicitly designed for treatment that can turn on and off, generalizing the event-study approach by defining the event as the period a unit's treatment first *changes*, whichever direction. Practically: check the reciprocal-agreement panel for reversals first (rare, likely absorbing, so Callaway–Sant'Anna should be fine there); check the level-1-only spells for reversals before choosing an estimator for that piece, since AGOA-style suspensions make this the one part of the design where the switching-robust estimator is likely necessary rather than optional.
