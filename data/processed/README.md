...
| `within`, `structural_change`, `employment_share_0`/`_1`, `productivity_0`/`_1`, `contribution_productivity_0`/`_1`, `short_series` | Carried over unchanged from the sector-level decomposition file (or synthesized for `"Total"` rows as described above) — `within`/`structural_change` are growth-rate contributions (see above), `productivity_0`/`_1` are each sector's own standalone productivity level, `contribution_productivity_0`/`_1` (`LP_b`) is that sector's own contribution to country-wide productivity |
...
The §6.3 Design A component regressions above use 3-year and 5-year intervals because differencing amplifies noise in flow variables — but the §6.4.B baseline (`ln_productivity`, the sector-level *level*, not a flow) was originally run only on the annual panel. This file asks two related questions. **First**: does interval resolution matter for the standalone `ln(productivity)` regression, re-run on each broad sector's own standalone productivity at the start of each 3-year and 5-year interval (`productivity_1`/`country_productivity_1` — point-in-time at the calendar-grid edge year, `Q_b/L_b`). **Second, added on request**: does `Reciprocal` affect `LP_b` (`contribution_productivity_1`, that sector's own contribution to country-wide productivity, `Q_b/L_country`) directly, rather than only implicitly through the `within`/`structural_change` regressions above, which use `LP_b` purely as an internal normalizing denominator. Both use the same `twfe()` helper and fixed-effect structure as the annual case. 6 rows: 2 interval widths × (2 levels for standalone productivity [sector/country] + 1 sector-level-only row for `LP_b` — a country-level `LP_b` regression would just reproduce the country-level standalone-productivity row, since at country level the two quantities are identical by construction), baseline sample only.

| Column | Meaning |
|---|---|
| `interval` | `interval3` or `interval5` |
| `level` | `sector` (Agriculture/Manufacturing/Services) or `country` (the `"Total"` pseudo-sector rows; `contribution_productivity` rows are sector-level only) |
| `outcome` | `productivity` (`ln(productivity_1)`/`ln(country_productivity_1)`, standalone) or `contribution_productivity` (`ln(contribution_productivity_1)`, `LP_b`) |
| `beta`, `se`, `p` | The `reciprocal_interval` coefficient on the log outcome, cluster-robust (by country) SE, and p-value |
| `n`, `n_clusters` | Regression sample size and number of country clusters |

**Headline finding 1: interval smoothing does not clearly help the standalone productivity regression, unlike for the §6.3.A flow components.** The point estimate is positive at every resolution (0.064 to 0.157) and never reaches significance, matching the annual baseline's own null result (β = 0.149, SE = 0.121, p = 0.22 — see `twfe_baseline_results.csv` above). Precision is a wash rather than a clean improvement: 3-year sector-level is the one specification that's *more* precise than annual (SE 0.118 vs. 0.121), but 3-year country-level, 5-year sector-level, and 5-year country-level are all *less* precise, with 5-year country-level the worst of the five (SE 0.139). This is the expected pattern for a level variable, which does not carry the differencing-amplified noise that motivated interval-smoothing the flow variables in the first place — confirmed directly, not just assumed.

**Headline finding 2: `LP_b` gets a genuinely different answer from standalone productivity, not just a noisier version of the same one.** Both `contribution_productivity` point estimates sit almost exactly at zero (3-year: +0.0004, p ≈ 0.998; 5-year: -0.0148, p ≈ 0.920) — far smaller than `productivity_1`'s own positive 0.157/0.129 at the same two resolutions. Since `LP_b = productivity_b × employment_share_b` by construction, this is consistent with the employment-share side offsetting whatever positive productivity signal exists — though this specification alone can't distinguish a genuine reallocation effect from noise in a two-part ratio. Either way, "does a sector produce more per worker" and "does that sector end up contributing more to the country's overall output per worker" are different questions with different answers here.
